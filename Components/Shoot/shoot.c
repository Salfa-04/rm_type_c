...
void shoot_init(void) {
  const fp32 Trigger_speed_pid[3] = {
      TRIGGER_SPEED_PID_KP,
      TRIGGER_SPEED_PID_KI,
      TRIGGER_SPEED_PID_KD,
  };

  const fp32 Trigger_angle_pid[3] = {
      TRIGGER_ANGLE_PID_KP,
      TRIGGER_ANGLE_PID_KI,
      TRIGGER_ANGLE_PID_KD,
  };

  const fp32 Fric1_speed_pid[3] = {
      FRIC1_KP,
      FRIC1_KI,
      FRIC1_KD,
  };

  const fp32 Fric2_speed_pid[3] = {
      FRIC2_KP,
      FRIC2_KI,
      FRIC2_KD,
  };

  shoot_control.shoot_mode = SHOOT_STOP;     // 遥控器指针
  shoot_control.shoot_rc = getp_remtctrl();  // 拨弹电机数据
  ...
  // 拨弹盘 速度环 PID
  PID_init(&shoot_control.trig_pid_speed, PID_POSITION, Trigger_speed_pid,
           TRIGGER_ANGLE_PID_MAX_KP, TRIGGER_ANGLE_PID_MAX_KI);

  // 拨弹盘 角度环 PID
  PID_init(&shoot_control.trig_pid_angle, PID_POSITION, Trigger_angle_pid,
...
/// 摩擦轮慢慢停止
static bool_t shoot_will_down = 0;
...
int16_t shoot_control_loop(void) {
  ...
  if (shoot_control.shoot_mode == SHOOT_STOP) {
    ...
    PID_calc(&shoot_control.trig_pid_speed,
             MOTOR_TO_TRIGGER_RPM * shoot_control.trig_motor->speed_rpm,
  ...
  if (shoot_control.shoot_mode == SHOOT_BULLET_STOP) {
    ...
    PID_calc(&shoot_control.trig_pid_speed, shoot_control.trig_motor->speed_rpm,
             shoot_control.trig_pid_angle.out);
    shoot_control.trig_current = (int16_t)(shoot_control.trig_pid_speed.out);
    if (fabs(shoot_control.trig_speed) < 0.001) {
  ...
  return shoot_control.trig_current;  // 返回播弹电机电流值
}
...
static void shoot_set_mode(void) {
  ...
  else if (switch_is_up(RC_LEVER_SHOOT) && !switch_is_up(last_s) &&
           shoot_control.shoot_mode != SHOOT_STOP) {
    shoot_control.shoot_mode = SHOOT_STOP;
  }  // 遥控器拨杆位于中档时, 可使用键盘控制; 此处使用 Q 键开启摩擦轮
  else if (switch_is_mid(RC_LEVER_SHOOT) &&
           (RC_KEY_HOLD() & SHOOT_ON_KEYBOARD) &&
           shoot_control.shoot_mode == SHOOT_STOP) {
    shoot_control.shoot_mode = SHOOT_READY_FRIC;
  }  // 遥控器拨杆位于中档时, 使用 E 键关闭摩擦轮
  else if (switch_is_mid(RC_LEVER_SHOOT) &&
           (RC_KEY_HOLD() & SHOOT_OFF_KEYBOARD) &&
           shoot_control.shoot_mode != SHOOT_STOP) {
    shoot_control.shoot_mode = SHOOT_STOP;
  }
  ...
  if (shoot_control.shoot_mode == SHOOT_READY_FRIC &&
    ...
    shoot_control.shoot_mode = SHOOT_READY;
           ...
           shoot_control.key == SWITCH_TRIGGER_OFF) {
    shoot_control.shoot_mode = SHOOT_READY_BULLET;
  }  // 发射准备就绪, 判断发射状态
  else if (shoot_control.shoot_mode == SHOOT_READY) {
    // 下拨一次或鼠标按下一次, 开始射击
    if ((switch_is_down(RC_LEVER_SHOOT) && !switch_is_down(last_s)) ||
