...
static void trig_motor_fallback(int16_t);
...
void shoot_control_loop(void) {
  ...
  if (shoot_control.shoot_mode == SHOOT_STOP) {
    ...
    if (shoot_control.trig_button_hold == SWITCH_TRIGGER_OFF) {  /// 没有进弹
      ...
      // 判断电机是否被卡弹
      trig_motor_fallback(shoot_control.mot_trig->speed_rpm);
    } else {            /// 已经进弹
      can_trig_hold();  // 锁定拨弹盘
    }
    ...
    if (shoot_control.shoot_ready_trig_time >= BAKE_TIME_OVER) {
      can_trig_back();  // 拨弹盘电机反向转动角度

      shoot_control.shoot_mode = SHOOT_READY_BULLET;
      shoot_control.shoot_ready_trig_time = 0;
    }
  }
}
...
/// 判断电机是否被卡弹
static void trig_motor_fallback(int16_t speed) {
  if (abs(speed) < 2) {
    if (++shoot_control.shoot_ready_trig_time > BAKE_TIME_OVER) {
      shoot_control.shoot_ready_trig_time = BAKE_TIME_OVER;
