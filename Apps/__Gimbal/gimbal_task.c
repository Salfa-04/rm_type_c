...
#define gimbal_total_pid_clear(gimbal_clear)                                  \
  {                                                                           \
    gimbal_PID_clear(                                                         \
        &(gimbal_clear)->gimbal_yaw_motor.gimbal_motor_absolute_angle_pid);   \
    gimbal_PID_clear(                                                         \
        &(gimbal_clear)->gimbal_yaw_motor.gimbal_motor_relative_angle_pid);   \
    PID_clear(&(gimbal_clear)->gimbal_yaw_motor.gimbal_motor_gyro_pid);       \
                                                                              \
    gimbal_PID_clear(                                                         \
        &(gimbal_clear)->gimbal_pitch_motor.gimbal_motor_absolute_angle_pid); \
    gimbal_PID_clear(                                                         \
        &(gimbal_clear)->gimbal_pitch_motor.gimbal_motor_relative_angle_pid); \
 ...
 * @retval         none
 */
static void gimbal_PID_init(gimbal_PID_t *pid, fp32 maxout,
                            fp32 intergral_limit, fp32 kp, fp32 ki, fp32 kd);
 ...
 * @retval         pid 输出
 */
static fp32 gimbal_PID_calc(gimbal_PID_t *pid, fp32 get, fp32 set,
                            fp32 error_delta);
 ...
 * @retval         none
 */
static void calc_gimbal_cali(const gimbal_step_cali_t *gimbal_cali,
...
void gimbal_task(void const *args) {
  (void)args;
  ...
  while (1) {
    ...
    shoot_can_set_current =
        shoot_control_loop();  // 射击任务控制循环//弹丸发射（把发射拿出来加上PID）
...
static void gimbal_init(gimbal_control_t *init) {
  ...
  gimbal_control.gimbal_pitch_motor.max_relative_angle =
      0.53000000000f;  // max_pitch;
  gimbal_control.gimbal_pitch_motor.min_relative_angle =
      -0.3224369850f;  // min_pitch;
  // 修改yaw轴初始yaw目标值。

  static const fp32 Pitch_speed_pid[3] = {
      PITCH_SPEED_PID_KP, PITCH_SPEED_PID_KI, PITCH_SPEED_PID_KD};
  static const fp32 Yaw_speed_pid[3] = {YAW_SPEED_PID_KP, YAW_SPEED_PID_KI,
                                        YAW_SPEED_PID_KD};
  // 电机数据指针获取
  ...
  gimbal_PID_init(&init->gimbal_yaw_motor.gimbal_motor_absolute_angle_pid,
                  ...
                  YAW_GYRO_ABSOLUTE_PID_KP, YAW_GYRO_ABSOLUTE_PID_KI,
  ...
  gimbal_PID_init(&init->gimbal_yaw_motor.gimbal_motor_relative_angle_pid,
                  ...
                  YAW_ENCODE_RELATIVE_PID_MAX_IOUT, YAW_ENCODE_RELATIVE_PID_KP,
                  YAW_ENCODE_RELATIVE_PID_KI, YAW_ENCODE_RELATIVE_PID_KD);
  PID_init(&init->gimbal_yaw_motor.gimbal_motor_gyro_pid, PID_POSITION,
           Yaw_speed_pid, YAW_SPEED_PID_MAX_OUT, YAW_SPEED_PID_MAX_IOUT);
  // 初始化pitch电机pid
  gimbal_PID_init(&init->gimbal_pitch_motor.gimbal_motor_absolute_angle_pid,
                  PITCH_GYRO_ABSOLUTE_PID_MAX_OUT,
                  PITCH_GYRO_ABSOLUTE_PID_MAX_IOUT, PITCH_GYRO_ABSOLUTE_PID_KP,
                  PITCH_GYRO_ABSOLUTE_PID_KI, PITCH_GYRO_ABSOLUTE_PID_KD);
  gimbal_PID_init(&init->gimbal_pitch_motor.gimbal_motor_relative_angle_pid,
                  ...
                  PITCH_ENCODE_RELATIVE_PID_KP, PITCH_ENCODE_RELATIVE_PID_KI,
                  PITCH_ENCODE_RELATIVE_PID_KD);
  PID_init(&init->gimbal_pitch_motor.gimbal_motor_gyro_pid, PID_POSITION,
           Pitch_speed_pid, PITCH_SPEED_PID_MAX_OUT, PITCH_SPEED_PID_MAX_IOUT);

  // 清除所有PID
  gimbal_total_pid_clear(init);

  gimbal_feedback_update(init);
  init->gimbal_yaw_motor.absolute_angle_set =
      init->gimbal_yaw_motor.absolute_angle;
  init->gimbal_yaw_motor.relative_angle_set =
      init->gimbal_yaw_motor.relative_angle;
  init->gimbal_yaw_motor.motor_gyro_set = init->gimbal_yaw_motor.motor_gyro;

  init->gimbal_pitch_motor.absolute_angle_set =
      init->gimbal_pitch_motor.absolute_angle;
...
static void gimbal_feedback_update(gimbal_control_t *feedback_update) {
  ...
  feedback_update->gimbal_pitch_motor.absolute_angle =
      -*(feedback_update->gimbal_INT_angle_point + INS_ROLL_ADDRESS_OFFSET);
 ...
 * @retval         none
 */
static void gimbal_mode_change_control_transit(
    gimbal_control_t *gimbal_mode_change) {
  if (gimbal_mode_change == NULL) {
    return;
  }
  // yaw电机状态机切换保存数据
  if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode !=
          GIMBAL_MOTOR_RAW &&
  ...
  } else if (gimbal_mode_change->gimbal_yaw_motor.last_gimbal_motor_mode !=
                 GIMBAL_MOTOR_GYRO &&
             gimbal_mode_change->gimbal_yaw_motor.gimbal_motor_mode ==
                 GIMBAL_MOTOR_GYRO) {
  ...
  // pitch电机状态机切换保存数据
  if (gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode !=
          GIMBAL_MOTOR_RAW &&
  ...
  } else if (gimbal_mode_change->gimbal_pitch_motor.last_gimbal_motor_mode !=
                 GIMBAL_MOTOR_GYRO &&
             gimbal_mode_change->gimbal_pitch_motor.gimbal_motor_mode ==
                 GIMBAL_MOTOR_GYRO) {
...
static void gimbal_set_control(gimbal_control_t *set_control) {
  ...
  if (set_control->gimbal_pitch_motor.gimbal_motor_mode == GIMBAL_MOTOR_RAW) {
    ...
    gimbal_absolute_angle_limit(&set_control->gimbal_pitch_motor,
                                -add_pitch_angle);
    gimbal_control.motor_mode_flag = 0;
  } else if (set_control->gimbal_pitch_motor.gimbal_motor_mode ==
             GIMBAL_MOTOR_ENCONDE) {
 ...
 * @retval         none
 */
static void gimbal_absolute_angle_limit(gimbal_motor_t *gimbal_motor,
                                        fp32 add) {
  ...
  if (gimbal_control.motor_mode_flag == 1) {
    // now angle error
    // 当前控制误差角度
    bias_angle = rad_format(gimbal_motor->absolute_angle_set -
                            gimbal_motor->absolute_angle);
    // relative angle + angle error + add_angle > max_relative angle
    // 云台相对角度+ 误差角度 + 新增角度 如果大于 最大机械角度
    if (gimbal_motor->relative_angle + bias_angle - add >
        gimbal_motor->max_relative_angle) {
      // 如果是往最大机械角度控制方向
      if (add > 0.0f) {
        // calculate max add_angle
        // 计算出一个最大的添加角度，
        add = gimbal_motor->max_relative_angle - gimbal_motor->relative_angle -
              bias_angle;
        if (gimbal_motor->relative_angle > gimbal_motor->max_relative_angle) {
          add = 0;
        }
      }
    } else if (gimbal_motor->relative_angle + bias_angle - add <
               gimbal_motor->min_relative_angle) {
      if (add < 0.0f) {
        add = gimbal_motor->min_relative_angle - gimbal_motor->relative_angle -
              bias_angle;
        if (gimbal_motor->relative_angle < gimbal_motor->max_relative_angle) {
          add = 0;
        }
...
static void gimbal_motor_absolute_angle_control(gimbal_motor_t *gimbal_motor) {
  ...
  gimbal_motor->current_set =
      PID_calc(&gimbal_motor->gimbal_motor_gyro_pid, gimbal_motor->motor_gyro,
...
static void gimbal_motor_relative_angle_control(gimbal_motor_t *gimbal_motor) {
  ...
  //		if(gimbal_control.motor_mode_flag ==0)
  //		{
  //			gimbal_motor->given_current =
  ///*-*/(int16_t)(gimbal_motor->current_set);
  //		}
...
static void J_scope_gimbal_test(void) {
  yaw_ins_int_1000 =
      (int32_t)(gimbal_control.gimbal_yaw_motor.absolute_angle * 1000);
  yaw_ins_set_1000 =
      (int32_t)(gimbal_control.gimbal_yaw_motor.absolute_angle_set * 1000);
  yaw_speed_int_1000 =
      (int32_t)(gimbal_control.gimbal_yaw_motor.motor_gyro * 1000);
  yaw_speed_set_int_1000 =
      (int32_t)(gimbal_control.gimbal_yaw_motor.motor_gyro_set * 1000);

  pitch_ins_int_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.absolute_angle * 1000);
  pitch_ins_set_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.absolute_angle_set * 1000);
  pitch_speed_int_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.motor_gyro * 1000);
  pitch_speed_set_int_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.motor_gyro_set * 1000);
  pitch_relative_angle_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.relative_angle * 1000);
  pitch_relative_set_1000 =
      (int32_t)(gimbal_control.gimbal_pitch_motor.relative_angle_set * 1000);
}

#endif

static void gimbal_PID_init(gimbal_PID_t *pid, fp32 maxout, fp32 max_iout,
...
static void gimbal_PID_clear(gimbal_PID_t *gimbal_pid_clear) {
  if (gimbal_pid_clear == NULL) return;

  gimbal_pid_clear->err = gimbal_pid_clear->set = gimbal_pid_clear->get = 0.0f;
  gimbal_pid_clear->out = gimbal_pid_clear->Pout = gimbal_pid_clear->Iout =
      gimbal_pid_clear->Dout = 0.0f;
}
