/**
  ...
  * @note       this is only controling 80 w power, mainly limit motor current
  set.
  *             if power limit is 40w, reduce the value
  JUDGE_TOTAL_CURRENT_LIMIT
  *             and POWER_CURRENT_LIMIT, and chassis max speed (include
...
void chassis_power_control(chassis_move_t *chassis_power_control) {
  ...
  if (toe_is_error(REFEREE_TOE)) {
    ...
    // power > 80w and buffer < 60j, because buffer < 60 means power has been
    // more than 80w
    ...
    } else  // 对应的是缓冲能量>50
    {
      ...
      if (chassis_power > WARNING_POWER)  // chassis_power>40
      {
        ...
        if (chassis_power < POWER_LIMIT)  // 40<chassis_power<80
        {
          ...
          // 缩小
          power_scale =
              (POWER_LIMIT - chassis_power) / (POWER_LIMIT - WARNING_POWER);
        ...
        total_current_limit = BUFFER_TOTAL_CURRENT_LIMIT +
                              POWER_TOTAL_CURRENT_LIMIT * power_scale;
      }
      // power < WARNING_POWER
      // 功率小于WARNING_POWER
      else {
        total_current_limit =
            BUFFER_TOTAL_CURRENT_LIMIT + POWER_TOTAL_CURRENT_LIMIT;
      }
    }
  }
