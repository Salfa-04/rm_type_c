...
/**
 * @brief          "chassis_move" valiable initialization, include pid
 * initialization, remote control data point initialization, 3508 chassis motors
 *                 data point initialization, gimbal motor data point
 * initialization, and gyro sensor angle point initialization.
...
/**
 * @brief          set chassis control mode, mainly call
 * 'chassis_behaviour_mode_set' function
...
/**
 * @brief          when chassis mode change, some param should be changed, suan
 * as chassis yaw_set should be now chassis yaw
...
/**
 * @brief          chassis some measure data updata, such as motor speed, euler
 * angle， robot speed
 ...
 * is set by "chassis_behaviour_control_set".
 *
 * @param[out]     chassis_move_update: "chassis_move" valiable point
 * @retval         none
 */
...
/**
 * @brief          control loop, according to control set-point, calculate motor
 * current, motor current will be sentto motor
...
void chassis_task(void const *pvParameters) {
  ...
  while (1) {
    ...
    // make sure  one motor is online at least, so that the control CAN message
    // can be received 确保至少一个电机在线， 这样CAN控制包可以被接收到
    if (!(toe_is_error(CHASSIS_MOTOR1_TOE) &&
          toe_is_error(CHASSIS_MOTOR2_TOE) &&
          toe_is_error(CHASSIS_MOTOR3_TOE) &&
          toe_is_error(CHASSIS_MOTOR4_TOE))) {
      // when remote control is offline, chassis motor should receive zero
      // current. 当遥控器掉线的时候，发送给底盘电机零电流.
...
static void chassis_init(chassis_move_t *chassis_move_init) {
  ...
  // 获取底盘电机数据指针，初始化PID
  for (i = 0; i < 4; i++) {
    chassis_move_init->motor_chassis[i].chassis_motor_measure =
        get_chassis_motor_measure_point(i);
    PID_init(&chassis_move_init->motor_speed_pid[i], PID_POSITION,
             motor_speed_pid, M3505_MOTOR_SPEED_PID_MAX_OUT,
             M3505_MOTOR_SPEED_PID_MAX_IOUT);
  }
  // initialize angle PID
  // 初始化角度PID
  PID_init(&chassis_move_init->chassis_angle_pid, PID_POSITION, chassis_yaw_pid,
 ...
 * @retval         none
 */
static void chassis_mode_change_control_transit(
    chassis_move_t *chassis_move_transit) {
  ...
  // 切入跟随底盘角度模式
  else if ((chassis_move_transit->last_chassis_mode !=
...
static void chassis_feedback_update(chassis_move_t *chassis_move_update) {
  ...
  // calculate chassis euler angle, if chassis add a new gyro sensor,please
  // change this code 计算底盘姿态角度, 如果底盘上有陀螺仪请更改这部分代码
...
/**
 * @brief          accroding to the channel value of remote control, calculate
...
void chassis_rc_to_control_vector(fp32 *vx_set, fp32 *vy_set,
  ...
  // deadline, because some remote control need be calibrated,  the value of
  // rocker is not zero in middle place, 死区限制，因为遥控器可能存在差异
  // 摇杆在中间，其值不为0
  rc_deadband_limit(
  ...
  // first order low-pass replace ramp function, calculate chassis speed
  // set-point to improve control performance
  // 一阶低通滤波代替斜波作为底盘速度输入
...
/**
 * @brief          设置底盘控制设置值,
 * 三运动控制值是通过chassis_behaviour_control_set函数设置的
 * @param[out]     chassis_move_update:"chassis_move"变量指针.
 * @retval         none
 */
static void chassis_set_contorl(chassis_move_t *chassis_move_control) {
  ...
  if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW) {
    ...
    sin_yaw =
        arm_sin_f32(-chassis_move_control->chassis_yaw_motor->relative_angle);
    cos_yaw =
        arm_cos_f32(-chassis_move_control->chassis_yaw_motor->relative_angle);
    ...
    chassis_move_control->wz_set =
        /*-*/ PID_calc(&chassis_move_control->chassis_angle_pid,
    ...
    chassis_move_control->vx_set = fp32_constrain(
        chassis_move_control->vx_set, chassis_move_control->vx_min_speed,
        chassis_move_control->vx_max_speed);
    chassis_move_control->vy_set = fp32_constrain(
        chassis_move_control->vy_set, chassis_move_control->vy_min_speed,
        chassis_move_control->vy_max_speed);
  } else if (chassis_move_control->chassis_mode ==
             CHASSIS_VECTOR_FOLLOW_CHASSIS_YAW) {
    ...
    // 计算旋转的角速度
    chassis_move_control->wz_set =
        PID_calc(&chassis_move_control->chassis_angle_pid, 0.0f, delat_angle);
    // speed limit
    // 速度限幅
    ...
    // 设置控制相对云台角度
    chassis_move_control->chassis_relative_angle_set =
        rad_format(angle_set);  // 弧度格式化为-PI~PI
    // calculate ratation speed
    // 计算旋转PID角速度
    // 底盘跟随云台，底盘相对与云台应加负号
    // 角度期望值为0
    ...
    static fp32 const sin_add_time =
        0.002;  // PI * 0.5f * configTICK_RATE_HZ / CHASSIS_CONTROL_TIME_MS;
...
static void chassis_vector_to_mecanum_wheel_speed(const fp32 vx_set,
                                                  ...
                                                  const fp32 wz_set,
                                                  fp32 wheel_speed[4]) {
  // because the gimbal is in front of chassis, when chassis rotates, wheel 0
  // and wheel 1 should be slower and wheel 2 and wheel 3 should be faster
  // 旋转的时候， 由于云台靠前，所以是前面两轮 0 ，1 旋转的速度变慢， 后面两轮
  // 2,3 旋转的速度变快
  wheel_speed[0] =
      -vx_set - vy_set + (-1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
  wheel_speed[1] =
      vx_set - vy_set + (-1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
  wheel_speed[2] =
      vx_set + vy_set + (-1.0f) * MOTOR_DISTANCE_TO_CENTER * wz_set;
